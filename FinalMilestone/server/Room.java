...
public class Room extends BaseGamePanel implements AutoCloseable {
		...
		try {
			if (message.indexOf(COMMAND_TRIGGER) > -1) {
				...
				case "flip":
					...
					if (coin == 1) {
						response = "<u><b> LANDED TAILS!! <b><u>";
					} else {
						response = "<u><b> LANDED HEADS!! <b><u>";
					}
					break;
				case "roll":
					int dice = (int) ((Math.random() * 6) + 1);
					response = "<u><b> ROLLED " + String.valueOf(dice) + "!<b><u>";
					break;
				...
				case "pm":
					...
					for (int i = 0; i < words.length; i++) {
						for (int x = 0; x < 2; x++) {
							words[x].trim().toLowerCase();
							clients.add(words[x]);
							message = message.replace(words[x], "");
						}
					}
				...
				for (int i = 0; i < words.length; i++) {
					/*
					 * if (words[i].indexOf("!") > -1) { List<String> clients = new
					 * ArrayList<String>(); for (int x = 0; x < 2; x++) { words[i] =
					 * words[i].replaceAll("!", ""); clients.add(words[i]);
					 * 
					 * } sendPrivateMessage(client, clients, message); response = null; break;
					 * 
					 * }
					 */
					if (words[i].indexOf("@") > -1) {
						words[i] = words[i].replaceAll("@", "");
						if (!client.isMuted(words[i])) {
							client.mute(words[i]);
							return "<b>" + words[i] + " IS NOW MUTED BY " + client.getClientName() + "</b>";

						} else {
							client.unMute(words[i]);
							return "<b>" + words[i] + " IS NOW UNMUTED BY " + client.getClientName() + "</b>";
						}
					}
					if (words[i].indexOf("*") > -1) {
					...
					if (words[i].indexOf("~") > -1) {
						if (words[i].lastIndexOf("*") >= -1) {
							...
							if (x == 1) {
								...
								words[i] = "<font color =green>" + words[i] + "</font>";
							} else if (x == 3) {
								words[i] = "<font color =blue>" + words[i] + "</font>";
							} else if (x == 4) {
								words[i] = "<font color =purple>" + words[i] + "</font>";
							} else if (x == 5) {
								words[i] = "<font color =yellow>" + words[i] + "</font>";
							}
						}
					}
				}
	...
	private void send(String string, String response) {
	...
	protected void sendMessage(ServerThread sender, String message) {
		...
		while (iter.hasNext()) {
			...
			/*
			 * if (client.client.isPrivate(sender.getClientName())) { boolean messageSent =
			 * client.client.send(sender.getClientName(), message); if (!messageSent) {
			 * iter.remove(); } }
			 */
		}
	}
