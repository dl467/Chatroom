...
public class ServerThread extends Thread {
	...
	List<String> privateClients = new ArrayList<String>();

	public boolean isMuted(String clientName) {
		return mutedClients.contains(clientName);
	}

	public boolean isPrivate(String clientName) {
		return privateClients.contains(clientName);
	}

	public void privated(String client) {
		if (!isPrivate(client)) {
	...
	public void unPrivated() {
		privateClients.clear();
		System.out.println(privateClients);

	}

	public void mute(String client) {
		if (!isMuted(client)) {
			mutedClients.add(client);
			System.out.println(mutedClients);
		}
	}

	public void unMute(String client) {
		if (isMuted(client)) {
	...
	private boolean sendOnIsMuted(String clientName, boolean isMuted) {
		// TODO Auto-generated method stub
		Payload payload = new Payload();
		payload.setPayloadType(PayloadType.MUTELIST);
		payload.setClientName(clientName);
		if (mutedClients.contains(clientName)) {
			isMuted = true;
	...
	private void processPayload(Payload p) {
		...
		case JOIN_ROOM:
			...
			break;
		case MUTELIST:
			sendOnIsMuted(p.getClientName(), p.getMuted());
			break;
		default:
