...
public class GamePanel extends BaseGamePanel implements Event {
	...
	public void onIsMuted(String clientName, boolean isMuted) {
		...
		while (iter.hasNext()) {
			Player p = iter.next();
			if (p.getName().equalsIgnoreCase(clientName)) {
				if (isMuted) {
					p.setName("<font color=red>" + clientName + "</font>");
				} else {
					p.setName("<font color=black>" + clientName + "</font>");
				}
				break;
			}
		}
	 ...
	 */
	/*
	 * private void applyControls() { if (myPlayer != null) { int x = 0, y = 0; if
	 * (KeyStates.W) { y = -1; } if (KeyStates.S) { y = 1; } if (!KeyStates.W &&
