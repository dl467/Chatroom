...
public class ClientUI extends JFrame implements Event {
	...
	void createUserInputScreen() {
		...
		JLabel userpassword = new JLabel("Password:"); // HERE
	...
	void addMessage(String str) {
		...
		textArea.add(entry);
		int txt = textArea.getComponentCount();
		for (int i = 0; i < txt; i++) {
			JEditorPane text = (JEditorPane) textArea.getComponent(i);
			if (text != null) {
				String mss = text.getText();
	...
	public void onSuccessfulMuteOrUnmute(String clientName, boolean isMuted) {
		Iterator<User> iter = users.iterator();
		while (iter.hasNext()) {
			User u = iter.next();
			if (u.getName().equalsIgnoreCase(clientName)) {
				if (isMuted) {
					u.setName("<font color=gray>" + clientName + "</font>");
				} else {
					u.setName("<font color=black>" + clientName + "</font>");
				}
				u.invalidate();
				u.repaint();
				break;
